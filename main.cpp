...
#pragma region エラーと警告の抑制
		...
		D3D12_MESSAGE_ID denyIds[] = {
			...
			//Windows11でのXGIデバッグレイヤーとDX12デバッグレイヤーの相互作用バグによるエラーメッセージ
			D3D12_MESSAGE_ID_RESOURCE_BARRIER_MISMATCHING_COMMAND_LIST_TYPE

		};
		//抑制するレベル
		D3D12_MESSAGE_SEVERITY serverties[] = { D3D12_MESSAGE_SEVERITY_INFO };
		D3D12_INFO_QUEUE_FILTER filter{};
		filter.DenyList.NumIDs = _countof(denyIds);
		filter.DenyList.pIDList = denyIds;
		filter.DenyList.NumSeverities = _countof(serverties);
		filter.DenyList.pSeverityList = serverties;
		//指定したメッセージ表示を抑制する
...
#pragma region TransitionBarrier
			...
			//バリアを春大将のresource　現在のバックバッファに対して行う
